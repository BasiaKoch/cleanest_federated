...
\caption[How the three optimisers differ]{How the three optimisers differ. All
three share the same broadcast--local-train--aggregate loop; only the highlighted
box changes. FedAvg performs plain weighted averaging of client updates; FedProx
adds a proximal penalty to the \emph{local} objective, constraining client drift
from the global model; and FedNova \emph{normalises} aggregation by each client's
effective local work $\tau_i$, correcting bias from unequal local computation. The
small notes preview the regime-dependent behaviour examined in
Chapter~\ref{ch:results}; they are not results.}
...
$n = 10$ paired seeds (FedAvg vs FedProx, mean $\pm 1$ SEM bands). By
...
organises the six experiments, and Figure~\ref{fig:regime-map-summary} summarises
the resulting map across the regimes studied.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{F_regime_map_summary.pdf}
\caption[Regime map: synthesis of the thesis findings]{Regime map: a qualitative synthesis of Chapter~\ref{ch:results}, not new
