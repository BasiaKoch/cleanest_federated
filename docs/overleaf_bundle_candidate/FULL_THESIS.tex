...
Images are loaded from \texttt{dermamnist\_64.npz} via the loader at
\texttt{mnist\_dermnist/data/load.py}: NHWC uint8 arrays are permuted to
CHW float, scaled to $[0, 1]$, bilinearly resized to $28 \times 28$, and
...
The construction is fixed in code (\texttt{data/partition.py}) and
...
The partition is deliberately structured. The
partition-generation comment in \texttt{data/partition.py} states that
it was \emph{``designed to maximise FedProx's advantage on the
...
FedAvg \citep{mcmahan2017fedavg} is the baseline. Clients run $E$
epochs of local SGD; the server aggregates by sample-weighted mean
$w_{t+1} = \sum_i (n_i / N) w_i^{t+1}$ (\texttt{fl/aggregation.py}).
...
to zero and the FedProx code path becomes bit-identical to FedAvg: the
implementation gates the proximal branch on \texttt{proximal\_mu > 0}
(see \texttt{mnist\_dermnist/fl/local\_train.py}) and a regression test
(\texttt{tests/test\_mu\_zero\_equals\_fedavg.py}) verifies this
...
The implementation (\texttt{fl\_flower/strategy\_fednova.py}) uses the
momentum-aware normaliser $a_i = (\tau_i(1-m) - m(1-m^{\tau_i})) /
...
$a_{\textrm{eff}} = \sum_i p_i a_i$;
\texttt{tests/test\_fednova\_aggregation.py} provides a
brute-force-vs-closed-form check. FedNova is the work-normalisation
...
latency). The schedule generator (\texttt{fl/system\_het.py}) has three
...
(\texttt{mnist\_dermnist/fl\_flower/strategy\_straggler\_dropping.py}),
...
(implementation in \texttt{fl/class\_imbalance.py}): plain cross-entropy
...
Two runtimes are used. A \emph{pure-PyTorch reference loop}
(\texttt{fl/server\_loop.py}) provides cross-runtime corroboration for
the headline statistical-heterogeneity experiment. The
\emph{Flower simulation} (framework $1.23$ \citep{beutel2020flower},
under \texttt{fl\_flower/}) is the production-grade runtime carrying
every other experiment and all reported headline numbers. Where the
two runtimes disagree on magnitude (engineered partition,
§\ref{sec:statistical-heterogeneity}), Flower is central and the
pure-PyTorch estimate is direction-only corroboration.
...
regenerated summaries, never to stale committed CSVs. Unit tests under
\texttt{tests/} guard the FedProx proximal term, the
...
best-validation checkpoint}, computed on the global test set
(\texttt{fl/evaluation.py}). Macro-F1 weights all seven classes equally
...
calibration metrics are not used. The FedProx code path includes the
proximal penalty in the logged \texttt{train\_loss}
(\texttt{fl/local\_train.py}); cross-algorithm comparisons therefore
use test macro-F1, never logged train-loss.
