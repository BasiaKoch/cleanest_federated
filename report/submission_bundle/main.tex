...
Several claims are deliberately out of scope. This is a controlled benchmark
simulation, not a hospital deployment: no wall-clock deadlines, privacy or
communication-cost budgets, or clinical validation. The optimiser set is not an exhaustive FL
...
stragglers ($n=10$, outside the stable-$\tau$ assumption of
\citet{wang2020fednova}). Test macro-F1; the mechanism reading is given in the
text.}
...
\item The L1/L4 partitions and the drop/straggler protocols are stylised stress
tests chosen to isolate method behaviour; the regime map describes behaviour at
these boundaries rather than naturalistic deployment.
...
pilots labelled as such) because compute availability varied. Seed tiers are
reported transparently per experiment; the $n = 3$ probes are directional and not
significance-tested.
\item The heterogeneity ladder is an exploratory single-seed pilot; multi-seed
evidence exists only at L4 ($n = 3$).
...
\item The FedNova random-$\tau$ collapse is an observed failure mode; isolating
its mechanism --- e.g.\ varying the straggler fraction and $\tau$-variance
independently --- is future work.
