...
$a_{\textrm{eff}} = \sum_i p_i a_i$; both FedNova's asymmetry regimes lie outside
...
Dependencies are pinned in \texttt{requirements.txt} (tested on Python~3.14); the
package installs with \texttt{pip install -e .}\ from the repository root.

\paragraph{Reproduction.}
The report's tables and figures rebuild from the saved per-run artefacts without
re-training. \texttt{bash infra/local/commands.sh sanity} runs the implementation
guards (the FedProx $\mu=0\equiv$ FedAvg equivalence, the proximal term, the
FedNova normaliser, and straggler handling), and
...
loops under \texttt{fl/} and \texttt{fl\_flower/}, the partitioner, and the
