...
$\gamma$-inexact cell, so the $+0.111$ macro-F1 attribution to
$\gamma$-inexact handling is for the \emph{FedProx + $\gamma$-inexact
...
combinations the global model collapses to predicting mel-nevi for
essentially every test image, recovering the raw-prevalence accuracy
floor while macro-F1 falls to the $0.087$--$0.115$ range.
...
Two federation sizes are used. The \emph{7-client federation} carries
the headline 10-seed benchmarks (IID, engineered balanced-paired,
Dirichlet $\alpha = 0.1$, specialist 1-of-7, Flower $C = 1$ baseline)
and the random-straggler system-het comparisons including the FedNova
random-$\tau$ collapse experiment. The \emph{2-client federation}
carries the controlled mechanism probes that need a clean rare-client
vs dominant-client interaction: L1 and L4 four-condition
decompositions, perfect-storm, LR-asymmetry envelope, weighted-CE/focal
comparators, asymmetric $\mu$, node-pinned L4, ladder pilot. No claim
of cross-scale generalisation between the two federation sizes is made.

Two training regimes are used. \emph{Regime A} (7-client default): SGD
with momentum $0.9$, batch $32$, $\textrm{lr} = 0.01$, weight decay
$0$, $E = 20$, $R = 150$. \emph{Regime B} (Li-style plain-SGD,
2-client mechanism probes): momentum $0$, batch $10$, same lr / wd /
$E$ / $R$. The four-condition decomposition additionally fixes Client
1's $E_{\textrm{straggler}} = 5$. Each experiment's regime is stated in
the corresponding Results section. The L0--L4 heterogeneity ladder is
a two-client pilot but uses Regime~A-style batch and momentum
(\texttt{batch=32, momentum=0.9}); it is used only for orientation and
is not a headline mechanism probe. The best-validation-checkpoint
...
This is the chapter's main statistical-heterogeneity benchmark and the
substrate for the headline FedAvg/FedProx comparison and the
$\mu$-sweep. Per-class allocations are fixed in code so that every
minority class (actinic, basal, dermatofibroma, melanoma, vascular) is
owned by exactly two clients; mel-nevi is distributed in
$\approx 670$-image shares across six clients with the seventh a
mel-nevi-only generalist. Per-client training sizes are
$964 / 963 / 1095 / 1094 / 1110 / 1108 / 673$. This is \emph{not} the
two-client $90/10$ partition (the $90/10$ label is reserved for the L4
mechanism partition below). The partition-generation comment in
\texttt{data/partition.py} states explicitly that the design was
\emph{``designed to maximise FedProx's advantage on the per-class
drift mechanism''} --- the small Flower-paired $\Delta$ reported in
§\ref{sec:statistical-heterogeneity} should therefore be read against
this FedProx-favouring framing.

\subsection{Robustness partitions}

Three further partitions test whether the engineered result is
artefactual: \emph{Dirichlet $\alpha = 0.1$} \citep{hsu2019dirichlet}
(standard stochastic non-IID, seed-dependent label skew);
\emph{specialist 1-of-7} (each minority class owned by exactly one
client, with per-client sample counts matched to the balanced-paired
partition, isolating pairing-vs-specialism from any quantity
confound); and the \emph{L0--L4 heterogeneity ladder} (single-seed
2-client pilot escalating statistical skew across five rungs;
orientation only).

\subsection{Two-client L1/L4 mechanism partitions}

The two-client L1 and L4 partitions are the cleanest mechanism control
in the chapter: same federation size and protocol family, differing
along a single axis --- whether the dropped or under-trained client
carries class signal the other lacks.

\emph{L4} is class-disjoint by design. Client~0 holds $100\,\%$ of the
four common classes (mel-nevi, benign-keratosis-like, actinic
keratoses, basal cell carcinoma) and $0\,\%$ of the rest. Client~1
holds $100\,\%$ of melanoma, dermatofibroma, and vascular lesions, and
$0\,\%$ of the rest. The sample split is $86/14$ rather than $90/10$
because all melanoma ($11.1\,\%$) sits on Client~1; the working file
\texttt{two\_client\_90\_10\_rare\_stress} carries the older $90/10$
name. The JS divergence between client distributions is $1.0$. L4 is
the substrate for the four-condition decomposition
(§\ref{sec:li-straggler}), perfect-storm, asymmetric-LR envelope
(§\ref{sec:lr-asymmetry}), and weighted-CE / focal-loss comparators
(§\ref{sec:rare-class-collapse}).

\emph{L1} is the matched-quantity control: same $86/14$ ratio but
every class stratified across both clients in near-global proportions;
JS divergence drops to $\approx 5 \times 10^{-6}$. There is quantity
skew but no unique rare-class signal to hold or lose. If the
four-condition $\gamma$-inexact contribution at L4 were a generic
straggler-policy gain it would survive at L1 too. The L4-vs-L1
contrast is the thesis's strongest single piece of evidence
for the mechanism interpretation.

Partitions define where rare-class information resides. The next two
sections (§\ref{sec:method-optimisers},
§\ref{sec:method-protocols}) define how optimiser and protocol choices
determine whether that information reaches the aggregated global model.

\begin{table}[H]
\centering
\small
\caption{Client-partition designs used in the chapter. Federation size
is fixed for each design; statistical-heterogeneity \emph{type} names
the property the partition controls, and the right-most column names
the Results section the design supports. The engineered balanced-paired
partition and the L1/L4 pair are the chapter's load-bearing partitions;
the others control for artefactual or runtime confounds.}
...
\hline
IID                             & 7-client   & none (negative control)         & Falsify a heterogeneity-free FedProx gain                              & §\ref{sec:statistical-heterogeneity}            \\
Engineered balanced-paired      & 7-client   & paired minority ownership       & Headline FedProx-vs-FedAvg and $\mu$-sweep                             & §\ref{sec:statistical-heterogeneity}, §\ref{sec:mu-sensitivity} \\
Dirichlet $\alpha = 0.1$        & 7-client   & stochastic Dirichlet skew       & Test engineered direction outside its framing                          & §\ref{sec:statistical-heterogeneity}            \\
Specialist 1-of-7               & 7-client   & singleton minority ownership    & Isolate pairing vs specialism from quantity                            & §\ref{sec:statistical-heterogeneity}            \\
Ladder L0--L4                   & 2-client   & escalating label skew           & Single-seed pilot; orientation only                                    & §\ref{sec:statistical-heterogeneity} (pilot)    \\
L1 (matched-quantity)           & 2-client   & quantity-only (JS $\approx 0$)  & Negative control for the $\gamma$-inexact mechanism                    & §\ref{sec:li-straggler}                         \\
L4 (class-disjoint)             & 2-client   & class-disjoint (JS $= 1$)       & Substrate for four-condition, perfect-storm, LR asymmetry, loss-side   & §\ref{sec:li-straggler}, §\ref{sec:lr-asymmetry}, §\ref{sec:rare-class-collapse} \\
\hline
\end{tabular}%
}
\end{table}
...
§\ref{sec:mu-sensitivity}, which evaluates sensitivity and locates
the empirical peak on this benchmark. Crucially, FedProx is \emph{not}
treated as a black-box: the four-condition decomposition in
§\ref{sec:li-straggler} separates the local proximal term from the
$\gamma$-inexact partial-update protocol FedProx is conventionally
paired with.

\subsection{FedNova}

FedNova \citep{wang2020fednova} keeps a FedAvg-style local solver but
normalises each client's update by its effective local work $\tau_i$,
addressing the objective-inconsistency problem. The implementation
(\texttt{fl\_flower/strategy\_fednova.py}) uses the momentum-aware
normaliser $a_i = (\tau_i(1-m) - m(1-m^{\tau_i})) / (1-m)^{2}$ (Wang
et al.\ 2020, Algorithm 1 and §3.3), reducing to $a_i = \tau_i$ when
$m = 0$. FedNova is tested under deterministic LR asymmetry and noisy
per-round $\tau_i$ in §\ref{sec:lr-asymmetry}; both regimes lie
outside the stable-$\tau$ assumption of the original analysis.
...
work per round (computational asymmetry, not wall-clock latency). The
schedule generator (\texttt{fl/system\_het.py}) has three active
...
The perfect-storm experiment keeps the L4 partition and Regime~B
(batch $10$, momentum $0$) of the four-condition decomposition but
switches to \texttt{random\_stragglers} at fraction $0.9$ and compares
FedAvg+drop with FedProx+$\gamma$-inexact at $\mu \in \{1.0, 0.01\}$
($n = 3$). Batch and momentum are unchanged; only schedule, severity,
and protocol arms differ. §\ref{sec:li-straggler} reports it as a
\emph{stress demonstration} of the same ordering at larger magnitude,
not as a causal decomposition.
...
weakening the rare-class client's update magnitude \emph{without
dropping it}. The envelope is the substrate for the FedNova
regime-dependence claim in §\ref{sec:lr-asymmetry}.

\subsection{Random-$\tau$ FedNova stress}

The companion FedNova stress (\texttt{system\_het\_random\_fednova})
uses the 7-client engineered balanced-paired partition under Regime~A
with \texttt{random\_stragglers} fraction $0.5$, $n = 10$. This is
\emph{not} an L4 experiment; the matching FedAvg/FedProx cells are in
\texttt{system\_het\_random}. The protocol violates the stable-$\tau$
...
whether it reaches aggregation. The next section introduces the
loss-side interventions used to test whether the FedProx-vs-FedAvg
gap is a disguised class-imbalance correction.
...
more loss
contribution; and focal loss \citep{lin2017focal} with $\gamma = 2$,
...
Flower production runs save stronger provenance where available:
\texttt{history\_*.csv} (per-round validation),
\texttt{test\_at\_best\_*.json} (selected-round metrics plus a
provenance dictionary --- git commit, hostname, framework version,
Python/PyTorch versions, ISO start/finish timestamps),
...
pure-PyTorch reference runs are less provenance-rich (no
\texttt{git\_commit}, no companion update-norm logs); §\ref{sec:statistical-heterogeneity}
notes this when comparing the two runtimes. Every numerical claim in
Chapter~\ref{ch:results} traces to raw artefacts or freshly regenerated
summaries, never to stale committed CSVs. Unit tests under \texttt{tests/} guard the FedProx proximal term,
the $\mu = 0 \Leftrightarrow$ FedAvg equivalence, the FedNova
normaliser, straggler dropping, partition determinism, and
framework-provenance fields.

\section{Evaluation metrics and statistical reporting}
\label{sec:method-metrics}

The primary metric is \emph{test macro-F1 at the best-validation
checkpoint}, computed on the global test set
(\texttt{fl/evaluation.py}); macro-F1 weights all seven classes
equally and is sensitive to rare-class collapse, whereas raw accuracy
saturates at the $\approx 66.9\,\%$ mel-nevi prevalence floor.
Secondary diagnostics are: per-class F1; per-class recall (the
confusion-matrix diagonal $P(\hat{y} = c \mid y = c)$, computed
downstream from \texttt{test\_predictions\_*.npz}); rare-class F1, the
unweighted mean of per-class F1 over $\mathrm{RARE\_IDX} =
\{\text{dermato}, \text{melanoma}, \text{vascular}\}$;
rare-to-mel-nevi misrouting, the unweighted mean over rare classes of
$P(\hat{y} = \text{nv} \mid y = c)$; and convergence curves /
per-client update norms (diagnostic only). Balanced accuracy is stored
as a sanity check but never used for ranking --- raw accuracy would
mis-rank the L4 5{:}1 LR-asymmetry cell (§\ref{sec:lr-asymmetry}).

Validation and test sets are global and class-stratified; client
partitions apply only to training. Validation macro-F1 selects the
best checkpoint round-by-round; test metrics are evaluated \emph{once}
at the selected checkpoint and are not used for selection.
Probabilities are not stored, so AUC and calibration metrics are not
used. The FedProx \texttt{train\_loss} log includes the proximal
penalty, so cross-algorithm comparisons use test macro-F1.

A \emph{protocol flip} is a sign reversal of $\Delta =
\mathrm{FedProx} - \mathrm{FedAvg}$ across the best-validation
checkpoint, the final round, and the last-$K$ mean; the audit is a
robustness check, not a result claim.
...
initialisation, partitioner shuffles, and data-loader RNG as far as
the implementation permits; cross-runtime determinism is not asserted,
which is why the pure-PyTorch / Flower comparison in
§\ref{sec:statistical-heterogeneity} is direction-only. The
\emph{node-pinned L4} configuration is a variance-and-provenance
control --- L4 four-condition cells re-run on a single compute node
with matched seeds --- not a partition design.
...
ignore without sacrificing accuracy. The client partitions (§\ref{sec:method-partitions})
control \emph{where} rare-class signal is located, from IID through
engineered and Dirichlet skew to the class-disjoint L4 substrate and
its quantity-only L1 control. The system-heterogeneity protocols
...
because the straggler schedule and the protocol arms both change at once
(fixed-straggler $\to$ random-$0.9$; four cells $\to$ three FedProx
$\mu$-varied arms vs FedAvg+drop) and the FedAvg+drop arm collapses too
...
obtains only $0.237 \pm 0.094$ macro-F1, far below the matching FedAvg
and FedProx cells (\texttt{system\_het\_random}, same partition and
schedule, $n = 10$: FA $0.493 \pm 0.028$, FP $0.510 \pm 0.017$). Three
of ten seeds collapse to macro-F1 $= 0.114$, the majority-class-only
baseline obtained by predicting mel-nevi for every test image.
