...
if [ -n "$non_flower_found" ]; then
  ...
  echo "        bash infra/slurm/submit_headline.sh"
  exit 3
fi
...
echo "When done:     bash infra/local/check_results.sh"
...
if [ ${#FAILED[@]} -ne 0 ]; then
  ...
  echo "  sbatch infra/slurm/slurm_template_flower.sh <algo> <mu> <seed> $LOCAL_EPOCHS $OUT_DIR $PARTITION"
fi
