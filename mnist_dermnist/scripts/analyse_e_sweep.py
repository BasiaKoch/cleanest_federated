"""Analyse the local-epoch (E) sweep on balanced_paired_7_clients —
FedProx mechanism test on the headline partition.
...
Partition rationale: balanced_paired_7_clients is the headline partition
where FedProx ALREADY shows a measurable advantage at E=20 (Δ macro-F1
≈ +0.027, n=10 paired seeds in results/headline/). The E-sweep tests
whether that established effect scales with local-epoch count as the
FedProx mechanism predicts, rather than betting on a partition where
the baseline Δ is statistically indistinguishable from zero.

Data sources:
  - mnist_dermnist/results/e_sweep/   (this sweep — new E values)
  - mnist_dermnist/results/headline/  (10-seed E=20 anchor, reused free)
...
DEFAULT_RESULTS_DIR = REPO_ROOT / "mnist_dermnist" / "results" / "e_sweep"
DEFAULT_ANCHOR_DIR = REPO_ROOT / "mnist_dermnist" / "results" / "headline"
...
def _plot_delta_vs_E(deltas: pd.DataFrame, summary: pd.DataFrame, out: Path) -> None:
    ...
    ax.set_title("FedProx mechanism: drift correction grows with local-epoch count\n"
                 "balanced_paired_7_clients partition; replication of Li et al. 2020 MLSys §5.2")
...
def main():
    ...
    if not runs:
        ...
        print("  MODE=pilot bash mnist_dermnist/scripts/submit_e_sweep.sh")
        return
