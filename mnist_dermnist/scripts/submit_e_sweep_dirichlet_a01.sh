...
# mnist_dermnist/results/dirichlet_a01/ as the E=20 anchor (zero new cost).
...
#       MODE=pilot bash mnist_dermnist/scripts/submit_e_sweep_dirichlet_a01.sh
#
#   FULL  (E∈{1,5,10,40} × 2 algos × 3 seeds = 24 jobs):
#       MODE=full bash mnist_dermnist/scripts/submit_e_sweep_dirichlet_a01.sh
#
#   FULL+ (also re-runs E=20 fresh in this dir instead of reusing dirichlet_a01/):
#       MODE=full_plus bash mnist_dermnist/scripts/submit_e_sweep_dirichlet_a01.sh
set -uo pipefail

REPO_ROOT=${REPO_ROOT:-/home/bk489/federated_clean/cleanest_federated}
MU=0.01
PARTITION=dirichlet_alpha01_7_clients
OUT_DIR=mnist_dermnist/results/e_sweep_dirichlet_a01
...
echo "  PYTHONPATH=. python mnist_dermnist/scripts/analyse_e_sweep_dirichlet_a01.py"
