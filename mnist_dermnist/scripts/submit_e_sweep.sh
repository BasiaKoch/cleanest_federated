...
# mnist_dermnist/results/headline/ via the analyser's --anchor-dir flag,
...
#       MODE=pilot bash mnist_dermnist/scripts/submit_e_sweep.sh
#
#   FULL  (24 jobs: 4 new E × 2 algos × 3 seeds):
#       MODE=full bash mnist_dermnist/scripts/submit_e_sweep.sh
#
#   FULL+ (30 jobs: also re-run E=20 here for sanity):
#       MODE=full_plus bash mnist_dermnist/scripts/submit_e_sweep.sh
set -uo pipefail
...
echo "  PYTHONPATH=. python mnist_dermnist/scripts/analyse_e_sweep.py"
