...
#
# Outputs:
#   mnist_dermnist/results/asymmetric_mu_L4/
#       test_at_best_{algo}_mu{...}{_muPC-c0m...-c1m...}_E20_s{42,123,456}.json
#
# Usage
# -----
#   bash mnist_dermnist/scripts/submit_asymmetric_mu_L4.sh                   # submit 12 jobs
#   SKIP_CONTROL=1 bash mnist_dermnist/scripts/submit_asymmetric_mu_L4.sh    # 9 jobs (drop ctrl)
#   DRY_RUN=1 bash mnist_dermnist/scripts/submit_asymmetric_mu_L4.sh         # print only
set -uo pipefail
...
OUTDIR="mnist_dermnist/results/asymmetric_mu_L4"
...
echo "  python mnist_dermnist/results/thesis_ready/scripts/analyse_asymmetric_mu_L4.py"
echo ""
if [ ${#FAILED[@]} -ne 0 ]; then
