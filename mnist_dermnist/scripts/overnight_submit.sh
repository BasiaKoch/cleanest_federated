...
# Usage
# -----
#   bash mnist_dermnist/scripts/overnight_submit.sh <ray_diag_jobid>
#   DRY_RUN=1 bash mnist_dermnist/scripts/overnight_submit.sh <ray_diag_jobid>
#
# Example:
#   sbatch mnist_dermnist/scripts/ray_diagnostic.sh
#   #   → Submitted batch job 29902842
#   bash mnist_dermnist/scripts/overnight_submit.sh 29902842
set -uo pipefail
...
if [ -z "${1:-}" ]; then
    ...
    echo "  jid=\$(sbatch --parsable mnist_dermnist/scripts/ray_diagnostic.sh)"
    echo "  bash $0 \$jid"
    exit 1
fi
...
for entry in "${SCRIPTS[@]}"; do
    ...
    full_path="$REPO_ROOT/mnist_dermnist/scripts/$script"
    if [ ! -x "$full_path" ]; then
...
echo "  cat mnist_dermnist/logs/ray_diag_${DIAG_JID}.out"
echo ""
