...
# To actually submit on the cluster:   DRY_RUN=0 bash mnist_dermnist/scripts/submit_fednova_stage3_expand.sh
set -uo pipefail
...
echo "  arms:    ${ARM_NAMES[*]}"
echo "  out:     mnist_dermnist/results/system_het_random_fednova_{${ARM_NAMES[*]// /,}}/  (merges with pilot data)"
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  echo "  DRY_RUN=0 bash mnist_dermnist/scripts/submit_fednova_stage3_expand.sh"
fi
echo ""
