...
# To actually submit on the cluster:   DRY_RUN=0 bash mnist_dermnist/scripts/submit_fednova_mechanism_fork_pilot.sh
set -uo pipefail
...
submit() {
  ...
  local template="$REPO_ROOT/mnist_dermnist/scripts/slurm_template_fednova.sh"
...
for i in "${!ARM_NAMES[@]}"; do
  ...
  out="mnist_dermnist/results/system_het_random_fednova_${name}"
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  echo "  DRY_RUN=0 bash mnist_dermnist/scripts/submit_fednova_mechanism_fork_pilot.sh"
fi
echo ""
echo "When runs complete, analyse with:"
echo "  for d in baseline mom0 tauclip320 servmom serverlr03; do"
echo "    PYTHONPATH=. python -m mnist_dermnist.analysis.amplification \\"
echo "      --dir mnist_dermnist/results/system_het_random_fednova_\$d \\"
echo "      --out mnist_dermnist/results/system_het_random_fednova_\$d/analysis/amplification"
echo "  done"
