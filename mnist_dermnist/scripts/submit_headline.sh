...
# in mnist_dermnist/results/headline/ are preserved unchanged as the
...
if [ ${#FAILED[@]} -ne 0 ]; then
  ...
  echo "  sbatch mnist_dermnist/scripts/slurm_template_flower.sh <algo> <mu> <seed> $LOCAL_EPOCHS $OUT_DIR $PARTITION"
fi
