# mnist_dermnist — FedAvg vs FedProx on DermaMNIST
...
`mnist_dermnist/results/thesis_ready/scripts/`. Run individual
analysers with `python <script>` from the repo root; representative
entry points:

```bash
PYTHONPATH=. python mnist_dermnist/results/thesis_ready/scripts/analyse_statistical_heterogeneity.py
PYTHONPATH=. python mnist_dermnist/results/thesis_ready/scripts/plot_engineered_convergence.py
PYTHONPATH=. python mnist_dermnist/results/thesis_ready/scripts/plot_engineered_per_class.py
```

Legacy entry points in `mnist_dermnist/analysis/` (`tables.py`,
