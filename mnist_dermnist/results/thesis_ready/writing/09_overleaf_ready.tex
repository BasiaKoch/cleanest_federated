...
\paragraph{Contemporary thesis-scale work.}
\citet{nedeljkovic2025fedprox} is the closest contemporary thesis-scale
reference: an MPhil dissertation on FedAvg vs FedProx for breast-lesion
segmentation in DCE-MRI under engineered class-imbalance partitions.
That work and this thesis share the paired comparison protocol but
diverge in task (segmentation vs classification), dataset (proprietary
DCE-MRI vs public DermaMNIST), heterogeneity construction (class
imbalance vs balanced-pair label skew), and inferential scope (two
algorithms vs three). The two studies together provide complementary
evidence on whether the proximal-anchor mechanism generalises across
medical imaging modalities and task families.
...
interventions. The contemporary thesis-scale work
\citep{nedeljkovic2025fedprox} provides a paired FedAvg-vs-FedProx contrast
on segmentation but does not include FedNova or system-heterogeneity
conditions. The present thesis combines the three contributions absent
...
(\texttt{scripts/submit\_flower\_C0\_baseline.sh}). Clients submit updated
...
FL framework                & Flower~1.29 (\texttt{flwr.simulation}) & \citet{beutel2020flower}; Ray-actor backend \\
...
inspired by \citet{nedeljkovic2025fedprox} §3.8.4 but carries an
...
same topic (e.g., \citet{nedeljkovic2025fedprox}) gets such visual interpretability
