...
python fl_dermamnist/results/thesis_ready/scripts/generate_all_figures.py
```
...
- **Raw outputs:** `fl_dermamnist/results/headline/` (per-run JSON + CSV files)
- **Aggregation:** `fl_dermamnist/results/headline/analysis/` (via `tables.py`)
...
- Per-round convergence CSVs for all 20 runs (those live in `fl_dermamnist/results/headline/`, ~40 files). The single-seed convergence figure (Fig. 8) draws on the Colab single-seed FedProx CSV recovered separately.
