...
# Usage
# -----
#   bash fl_dermamnist/scripts/overnight_submit.sh <ray_diag_jobid>
#   DRY_RUN=1 bash fl_dermamnist/scripts/overnight_submit.sh <ray_diag_jobid>
#
# Example:
#   sbatch fl_dermamnist/scripts/ray_diagnostic.sh
#   #   → Submitted batch job 29902842
#   bash fl_dermamnist/scripts/overnight_submit.sh 29902842
set -uo pipefail
...
if [ -z "${1:-}" ]; then
    ...
    echo "  jid=\$(sbatch --parsable fl_dermamnist/scripts/ray_diagnostic.sh)"
    echo "  bash $0 \$jid"
    exit 1
fi
...
for entry in "${SCRIPTS[@]}"; do
    ...
    full_path="$REPO_ROOT/fl_dermamnist/scripts/$script"
    if [ ! -x "$full_path" ]; then
