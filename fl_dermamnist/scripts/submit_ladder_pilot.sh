...
# Usage
# -----
#   bash fl_dermamnist/scripts/submit_ladder_pilot.sh             # submit
#   DRY_RUN=1 bash fl_dermamnist/scripts/submit_ladder_pilot.sh   # print only
#   SKIP_FEDNOVA=1 bash fl_dermamnist/scripts/submit_ladder_pilot.sh
set -uo pipefail
...
for i in "${!LEVELS[@]}"; do
    ...
    OUTDIR="fl_dermamnist/results/heterogeneity_ladder/L${LEVEL}_${PARTITION}"
...
echo "Outputs in:   fl_dermamnist/results/heterogeneity_ladder/L<level>_<partition>/"
echo ""
if [ ${#FAILED[@]} -ne 0 ]; then
