...
# To actually submit on the cluster:   DRY_RUN=0 bash fl_dermamnist/scripts/submit_fednova_stage3_expand.sh
set -uo pipefail
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  echo "  DRY_RUN=0 bash fl_dermamnist/scripts/submit_fednova_stage3_expand.sh"
fi
echo ""
