...
# To actually submit on the cluster:   DRY_RUN=0 bash fl_dermamnist/scripts/submit_fednova_mechanism_fork_pilot.sh
set -uo pipefail
...
submit() {
  ...
  local template="$REPO_ROOT/fl_dermamnist/scripts/slurm_template_fednova.sh"
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  echo "  DRY_RUN=0 bash fl_dermamnist/scripts/submit_fednova_mechanism_fork_pilot.sh"
fi
echo ""
