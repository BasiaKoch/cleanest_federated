...
echo "=== fl_dermamnist experiment status ==="
echo ""
...
for entry in "${SWEEPS[@]}"; do
  ...
  done=$(( $(count_complete "fl_dermamnist/results/$name") ))
  hist=$(( $(count_history  "fl_dermamnist/results/$name") ))
  status=" "
...
cent_done=$(( $(count_centralised "fl_dermamnist/results/centralised") ))
...
echo "  bash fl_dermamnist/scripts/analyse_all.sh"
echo ""
