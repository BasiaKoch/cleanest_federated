...
FED_DIR=fl_dermamnist/results/two_client_90_10_rare_stress
LOCAL_OUT=fl_dermamnist/results/small_hospital_local_only
FT_OUT=fl_dermamnist/results/small_hospital_finetune
mkdir -p "$LOCAL_OUT" "$FT_OUT" "$FED_DIR" fl_dermamnist/logs

FLOWER_TPL="$REPO_ROOT/fl_dermamnist/scripts/slurm_template_flower.sh"
SP_TPL="$REPO_ROOT/fl_dermamnist/scripts/slurm_template_single_process.sh"
...
--client-id $cid --num-epochs 5 --lr 0.001 --device cuda --out-dir $FT_OUT"
    JID=$(submit_single_process \
        ...
        "fl_dermamnist.experiments.run_finetune" \
        "$FT_FA_ARGS" \
        "$FA_JID") || true
...
echo "Logs at:      $REPO_ROOT/fl_dermamnist/logs/<job-name>_<jobid>.{out,err}"
echo ""
