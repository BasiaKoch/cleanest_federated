...
#       MODE=pilot bash fl_dermamnist/scripts/submit_e_sweep.sh
#
#   FULL  (24 jobs: 4 new E × 2 algos × 3 seeds):
#       MODE=full bash fl_dermamnist/scripts/submit_e_sweep.sh
#
#   FULL+ (30 jobs: also re-run E=20 here for sanity):
#       MODE=full_plus bash fl_dermamnist/scripts/submit_e_sweep.sh
set -uo pipefail
...
echo "  PYTHONPATH=. python fl_dermamnist/scripts/analyse_e_sweep.py"
