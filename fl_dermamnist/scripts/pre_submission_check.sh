...
#
# Usage:
#   bash fl_dermamnist/scripts/pre_submission_check.sh
#
# What this checks
# ----------------
...
SUBMIT_SCRIPTS=( $(ls "$REPO_ROOT/fl_dermamnist/scripts/"submit_*.sh "$REPO_ROOT/fl_dermamnist/scripts/"slurm_template*.sh "$REPO_ROOT/fl_dermamnist/scripts/"slurm_centralised.sh 2>/dev/null) )
...
step "flower_only_hpc_guard_test" \
    'PYTHONPATH=. python -m pytest fl_dermamnist/tests/test_no_pure_pytorch_hpc_submission.py -q'

# 8. Full pytest suite
step "pytest_full_suite" \
    'PYTHONPATH=. python -m pytest fl_dermamnist/tests/ -q'
