"""FedProx server (Flower simulation entry point).

Identical to scripts/fedavg/server.py except for:
  - imports FedProxClient instead of FedAvgClient
  - passes `mu` from configs.fedprox_config to each client
  - writes results to a separate directory

Usage:
    python -m new_clean_fed.scripts.fedprox.server
"""
...
from new_clean_fed.configs import fedprox_config as cfg
from new_clean_fed.src.fedprox.client import FedProxClient
from new_clean_fed.src.fedprox.model import CNNClassifier
from new_clean_fed.src.fedprox.prepare_data import (
    load_dermamnist, dirichlet_partition, print_distribution,
)
from new_clean_fed.src.fedavg.client import numpy_to_state_dict
...
def main():
    ...
    print(f"=== FedProx server ===")
    print(f"  Clients: {cfg.NUM_CLIENTS} | Rounds: {cfg.NUM_ROUNDS} | E: {cfg.NUM_LOCAL_EPOCHS} | μ: {cfg.MU}")
